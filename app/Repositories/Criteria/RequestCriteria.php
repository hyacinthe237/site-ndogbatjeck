    ...
    public function apply($model, $fieldsSearchable)
    {
        ...
        if (isset($orderBy) && !empty($orderBy)) {
            ...
            if(count($split) > 1) {
                /*
                 * ex.
                 * organizations|code -> join organizations on current_table.organization_id = organizations.id order by organizations
                 *
                 * organizations:name|organizations.code -> join organizations on current_table.name = organizations.id order
                 * by organizations.code (in case both tables have same column name)
                 */
        ...
        if (stripos($search, ':')) {
            ...
            foreach ($fields as $row) {
                try {
                    ...
                    $searchData[$field] = !empty($value) ? $value : null;
                } catch (\Exception $e) {
